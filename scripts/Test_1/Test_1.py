...
ROOT_DIR = f"{PRO_DIR}\\Test_1\\Output"
...
import logging
LogsPath = f"{PRO_DIR}\\Test_1\\Logs"  
...
def get_report(cursor, view, folder):
    logger.info(f"Creating Report for {view}") 
...
def do_the_graphics(meal_type, from_date, to_date, index1, index2, condition, value, folder):
    ...
    #dfrom = get_date_from_quarter(1, 0,Start_year)
    #dto = get_date_from_quarter(2, 3,End_year)
    if len(sys.argv) < 3:
...
def create_view_non_compliance_2plus(meal_type, from_date, to_date, condition, value, folder):
    select = f""" Select  "Report_Name","Employee_Org_Unit_4___Code","Employee" ,"Report_ID", 
...
def do_test1(value):
    ...
    #####TEST_1_2
    logger.info("Searching into attendees")
    ...
    Query = f""" CREATE VIEW Test_{meal_type}_2_2_temp as SELECT "Report_Name","Employee" ,"Report_ID", "Purpose", "Transaction_Date","Expense_Type","Bussiness_Group", 
                    "Total Meal Amount -- Approved Amount (rpt)",
...
def do_test_1_4():
    logger.info("Script Test_1_4 started")
    global Start_year
    global End_year
    global DateFrom
    global DateTo
    ...
    #print(select)
    cursor.execute(Query)
    Report_Folder= f"{folder}\\Report"
    os.makedirs(Report_Folder, exist_ok=True)
    get_report(cursor, f'Test_1_4', folder)
    from_old_file = f'{folder}\\Test_1_4.xlsx'
    to_new_file = f'{Report_Folder}\\Test_1_4_Company Events.xlsx'
    try:
...
def do_test_1_5():
    ...
    global DateFrom
    global DateTo
    logger.info("Script  Test_1_5 started")
    #print(len(sys.argv))
    if len(sys.argv) < 3:
        index1 = values.index(DateFrom)
        index2 = values.index(DateTo)
    ...
    folder = f"{ROOT_DIR}"
    os.makedirs(folder, exist_ok=True)
    select = f'''  select "Report__","Employee_Org_Unit_4___Code","Employee","Purpose","Country","Total_Report_Amount","Transaction_Date","Payment_Type","Expense_Type","Parent_Expense_Type",CAST("Approved_Amount__rpt_" as float ) AS "Approved_Amount__rpt_", DptMapping."Bussiness__Group"  as "Bussiness_Group" FROM CR_1 JOIN DptMapping 
    ON CR_1."Employee_Org_Unit_4___Code" =  DptMapping."Cost_Center___Key" where "Parent_Expense_Type" = "03. Employee Meals and Entertainment" AND "Expense_Type" = "Employee Awards/Recognition" AND "Transaction_Date" BETWEEN '{from_date}' AND '{to_date}'  ORDER BY CAST("Approved_Amount__rpt_" as float )  DESC '''
    # select = f'''Select * from CR_1 where "Transaction_Date" BETWEEN '{from_date}' AND '{to_date}' AND {condition} '''
    ...
    Report_Folder= f"{folder}\\Report"
    os.makedirs(Report_Folder, exist_ok=True)
    to_new_file = f'{Report_Folder}\\Test_1_5_Employee_Awards+Recognition.xlsx'
    try:
        os.remove(to_new_file)
    except:
        pass
...
def create_dashboard():
    ...
    shutil.copy(f"{csv_folder}\\Test_1_2_1.csv", f"{Dasboard}")
    shutil.copy(f"{csv_folder}\\Test_1_2_2.csv", f"{Dasboard}")
    shutil.copy(f"{csv_folder}\\Test_1_2_1.csv", f"{Dasboard}")
    shutil.copy(f"{Visualization}\\Test_1_1_1_Employee Meals and Entertainment.png", f"{Dasboard}")
    shutil.copy(f"{Visualization}\\Test_1_1_2_Employee Meals and Entertainment By Business Group.png", f"{Dasboard}")
...
def create_zip_archive():
    shutil.make_archive(f"{ROOT_DIR}\\Test_1", 'zip', f'{ROOT_DIR}\\Download')
